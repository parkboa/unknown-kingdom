...
2. **4단계 밴드가 하위 세 티어에서 비구속적인 것이 의도인가?** 초급 0~6 · 중급 0~5 ·
   고급 0~4는 모두 중앙 4:4를 포함해서, 그 세 티어는 교리 전과 **같은 자리**에 왕을 둔다.
   실제 사다리는 전문·마스터 2단뿐이다. 그리고 `openingWallStones: 4`는 다섯 티어가 동일한데,
   왕 자리만 티어별로 가르고 성벽 돌 수는 안 가른 것이 의도인지도 확인이 필요하다.
   *(초판에 있던 "4단계 모순"은 내 측정 착오였고 철회했다 — 위 정정 절 참조.)*
