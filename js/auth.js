...
export function createGuestAuth({ config, fetcher = fetch, native = false, secureStorage, locks = globalThis.navigator?.locks, captchaProvider }) {
  ...
  return {
    ...
    async deleteAccount() {
      if (native && !secureStorage) throw new Error('Secure session storage unavailable');
      const saved = native ? await secureStorage.get() : null;
      const response = await fetcher(`${native ? config.nativeBase : config.webBase}/account`, {
        method: 'DELETE', credentials: native ? 'omit' : 'include',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(native && saved?.value ? { refreshToken: saved.value } : {}),
        signal: AbortSignal.timeout(15000),
      });
      if (!response.ok) {
        let code = 'ACCOUNT_DELETION_FAILED';
        try {
          const result = await response.json();
          if (result?.error === 'NO_SESSION') code = 'NO_SESSION';
        } catch {}
      ...
      cached = undefined;
      if (native) await secureStorage.clear();
    },
  };
}
...
function onlineAuth() {
  ...
  // iOS injects app-local plugins here; registerPlugin belongs to the optional core JS runtime.
  const secureStorage = native ? globalThis.Capacitor.Plugins?.DaegukSession : undefined;
  const methods = native ? ['get', 'set', 'clear', 'requestCaptcha'] : [];
  if (methods.some(method => typeof secureStorage?.[method] !== 'function')) {
    const error = new Error('Native authentication bridge unavailable');
    error.code = 'NATIVE_BRIDGE_UNAVAILABLE';
    throw error;
  }
  auth = createGuestAuth({ config: AUTH_CONFIG, native, secureStorage,
    captchaProvider: () => requestGuestCaptcha(AUTH_CONFIG.captchaSiteKey, secureStorage) });
  return auth;
}
...
export async function getOnlineIdentity(socketUrl) {
  ...
  return onlineAuth().getSession();
}

export async function deleteOnlineAccount() {
  if (!AUTH_CONFIG.enabled) {
    const error = new Error('Account deletion unavailable');
    error.code = 'AUTH_DISABLED';
    throw error;
  }
  return onlineAuth().deleteAccount();
}
