...
test("Lobby tutorial opens directly and completes all browser-owned steps", async ({ page }) => {
  ...
  await expect(page.locator(".piece.red")).toHaveCount(1);
  await expect(page.locator(".piece.blue")).toHaveCount(1);
  await expect(page.locator(".red-king-zone-cell")).toHaveCount(9);
  await expect(page.locator(".blue-king-zone-cell")).toHaveCount(9);
  ...
  await expect(sanctuaryNext).toBeVisible();
  await expect(page.locator(".piece.red")).toHaveCount(5);
  await expect(page.locator(".piece.blue")).toHaveCount(5);
  await expect(page.locator(".cell.last-move")).toHaveCount(0);
  await expect(page.locator(".king-zone-cell")).toHaveCount(0);
  for (const coordinate of ["E1", "D2", "F2", "E3"]) await expectPiece(page, coordinate, "red");
  for (const coordinate of ["E7", "D8", "F8", "E9"]) await expectPiece(page, coordinate, "blue");
  await sanctuaryNext.click();
  ...
  await expectPiece(page, "B9", "red");
  await expectPiece(page, "E6", "red");
  await expectPiece(page, "D6", "blue");
  await expectPiece(page, "F6", "blue");
  await captureCell.click();
  await expect(sanctuaryNext).toBeVisible();
  await expect(page.getByRole("gridcell", { name: "E6", exact: true }).locator(".piece.red")).toHaveCount(0);
  await expectPiece(page, "E6", "blue");
  await sanctuaryNext.click();
  ...
  await expectPiece(page, "A1", "blue");
  await expectPiece(page, "B1", "red");
  await expectPiece(page, "B2", "blue");
  await expectPiece(page, "F5", "red");
  ...
  await expectPiece(page, "A9", "blue");
  await expectPiece(page, "B9", "red");
  await expectPiece(page, "B8", "blue");
  await expectPiece(page, "D7", "red");
  await expectPiece(page, "F9", "red");
  await expectPiece(page, "G8", "red");
  await enemyWallCell.click();
  await expect(page.locator("#tutorialMessage")).toHaveText("흑 병사는 백의 성벽에 막혀 포획되었습니다.");
  await expect(page.getByRole("gridcell", { name: "B9", exact: true }).locator(".piece.red")).toHaveCount(0);
  await expectPiece(page, "B9", "blue");
  await sanctuaryNext.click();
  ...
  await expectPiece(page, "E3", "red");
  await expectPiece(page, "F2", "red");
  await expectPiece(page, "G3", "red");
  ...
  await expectPiece(page, "D3", "blue");
  await expectPiece(page, "E3", "red");
  ...
  await expectPiece(page, "E1", "blue");
  await expectPiece(page, "D2", "blue");
  await expect(page.locator("#tutorialMessage")).toHaveText("외교관이 인접한 적을 모두 아군으로 바꾸었습니다.");
  await expect(sanctuaryNext).toBeVisible({ timeout: 5_000 });
  await sanctuaryNext.click();

  await expectPiece(page, "E4", "blue");
  await expectPiece(page, "G4", "red");
  ...
  await expect(wizardCell.locator(".piece.blue.special")).toHaveCount(1);

  await expectPiece(page, "H5", "red");
  ...
  await expect(page.getByRole("gridcell", { name: "H5", exact: true }).locator(".piece.red")).toHaveCount(0);
  ...
  await expectPiece(page, "F2", "blue");
  await expect(page.getByRole("gridcell", { name: "G5", exact: true }).locator(".piece")).toHaveCount(0);
  await expect(page.getByRole("gridcell", { name: "E2", exact: true }).locator(".piece.red.king")).toHaveCount(1);
  await expectPiece(page, "E3", "blue");
