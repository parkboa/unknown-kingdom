...
- 작업 전에 현재 모델과 작업 난이도·사용량의 적합성을 판단한다. 모델이 과하거나 부족하다고 판단되면 도구 실행·파일 수정 전에 현재 모델, 권장 모델, 간단한 이유를 알리고 "현재 모델로 계속 진행할까요?"라고 물어 사용자 답변을 기다린다. 같은 작업에서 이미 승인받았다면 반복해서 묻지 않는다. 모델명을 확인할 수 없으면 추측하지 말고 확인 불가라고 알린다. 모델을 임의로 변경하지 않는다.
- 새 대화에서 기존 작업을 이어갈 때는 `docs/WORK_HANDOFF.md`를 먼저 읽고, 해당 목표의 완료 기준과 다음 행동을 따른다. 중요한 결과나 중단 위치는 즉시 그 문서에 갱신한다. 다른 새 작업을 요청받으면 인증 작업을 자동으로 재개하지 않는다.
- 한 번에 하나의 에이전트만 파일을 수정한다.
- 다른 에이전트는 리뷰와 분석만 수행한다.
- 작업 결과에는 변경 파일과 검증 결과를 명시한다.
- 사용자가 직접 할 수 있는 화면 조작(예: 튜토리얼 진행·해금, CAPTCHA 확인, 실기기 앱 실행·종료·재실행, 수동 대국)은 사용자가 직접 한다. 해당 단계에 도달하면 필요한 조작과 확인할 결과만 짧게 안내하고 결과를 기다린다. 사용자가 맡기기 전에는 AI가 대신 반복 조작하거나 해금 상태를 우회하지 않는다. AI는 코드 분석·수정, 테스트 준비·결과 진단, 기록 정리를 진행하며 사용자 조작과 무관한 작업은 계속한다.
