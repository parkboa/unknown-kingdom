...
- [CODEX] 1a·1b·1c를 독립 플래그로 분리하고 현재 스키마의 고정 전술 shard에서 1·2단계를 짝지어 재검증했다 — 기본기 회귀는 없었으나 1a·1c의 마스터 `true_atari` −1, 1b의 수 변화 0, 2단계의 채점 변화 +0/−0으로 모두 운영 승격 보류. 대용량 결과는 `/tmp`에만 두고 소형 재현 설정과 요약만 기록했으며 전체 131개 테스트·전술 5/5·브라우저·모바일 빌드 통과.
- [CODEX] 1b 종국 영토와 2단계 성벽 왕 지뢰를 직접 자극하는 엔진 정답 국면을 추가했다 — 1b는 얕은 평가의 유일 승리수를 복구하지만 운영 마스터는 플래그 없이도 통과해 보류, 2단계는 장군·마법사·외교관 3/3에서 즉시 패배수를 막지 못해 재설계 판정. `npm run ai:probe-objectives`의 엔진 정답 검증 통과.
- [CLAUDE CODE] 3단계 `conversion_capture` 결함의 실제 원인을 계측으로 특정하고 고쳤다 — 보고된 원인(정적 점수를 `deepScore`가 덮어씀)은 사실과 달랐다. `scoreWithLookahead`는 `candidate.score`를 뿌리값으로 쓰므로 보너스는 심층 점수까지 살아남는다. 실제 원인은 둘: (1) `classifyMidgameCandidate`의 유닛 타입 고정 순위(general 3 > wizard 2 > diplomat 1)가 1229점으로 1위인 외교관 후보를 탐색 전에 삭제, (2) 살아남아도 최악 믿음 세계 혼합이 위협 900점을 절반으로 깎음(933→485). 전환 포획을 정책의 사실(`conversionCells`)로 올려 타입 순위 위에 두고, 같은 항을 최악 세계 점수에도 넣어 혼합이 깎지 못하게 했다. 왕 인접 칸은 제외했다 — 거기서는 장군·마법사도 똑같이 왕을 잡으므로 외교관만의 수가 아니다. 시험지 결함도 둘 고쳤다: 자기 왕이 단수인 국면은 구출이 강제라 제외하고, 정답 키에서 왕 인접 칸을 뺐다. **동일 국면 A/B(사전 고정 8국면, 시드 20260827) 마스터 2/8 → 8/8**, 신규 표본 프로브 4/4. 수정 전 코드에서 실패하는 회귀 테스트를 추가했다 — 정답 수뿐 아니라 후보가 풀에 살아남는 경로(`midgameTactics.conversionCapture`)까지 검증한다. 회귀: 전체 132개 테스트, 전술 5/5, Challenge 브라우저 1개, 모바일 빌드 통과. 브랜치 `claude/2026-08-27-diplomat-conversion`, 커밋 `687eef9`(수정·테스트)와 `2b4ff71`(시험지 키 정정).
