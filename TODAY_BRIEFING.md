...
- [CLAUDE CODE] 왕 성벽 오프닝을 다섯 티어에 반영하고 3단계 시험지 도구를 고쳤다 — `kingWallDistance` 밴드(초급 0~6 … 마스터 1 고정)와 `openingWallStones: 4`로 왕을 자기 성벽 옆에 세우고 상하좌우를 먼저 쌓는다. 행만 제약하고 열·순서는 티어 판단(마스터 `variance: 0` 유지). 표집기는 첫 6수 무작위 때문에 **지금까지 모든 측정의 왕이 어떤 티어도 두지 않았을 자리**에 있었다 — 티어가 자기 오프닝 5수를 두게 바꾸자 `breach_square`가 1650플라이당 5문제에서 250플라이당 3문제로 늘었다. 리포트에 국면과 선택지를 저장해 `--load-exam`·`--tier-override`로 재채점하도록 했다(165초 → 5초). **오프닝이 바뀌었으므로 이전 측정치는 전부 낡은 분포의 것이다.** 검증: 전체 132개 테스트, 전술 5/5, 브라우저 1개, 모바일 빌드 통과. 커밋 `d0c8517`(왕 교리), `ab33a68`(시험지 도구).

- [CLAUDE CODE] 새 분포에서 3단계 기준선을 내고 `breach_square`의 원인을 확정했다 — **믿음 기구가 아니라 탐색 깊이다.** 왕 성벽 오프닝 코퍼스(4시드 × 80문제, 국면 저장) 기준 초급 63 · 중급 64 · 고급 51 · 전문 38 · 마스터 43%인데, 깊이를 1로 낮추면 다섯 티어가 61~71%로 모인다(전문 +30pp, 마스터는 `localSearchDepth: 4`까지 낮춰야 +18pp). 믿음 기구를 끄면 오히려 나빠져(마스터 −12pp) 겁쟁이 가설은 반증됐고, 초급 0pp가 대조군이었다. 지연된 사실(반응이 터져야 값이 실현되는 수)을 깊은 탐색이 할인하는 것이며, 오늘 고친 전환 포획과 같은 병의 다음 증상이다 — 예외를 하나 더 파는 대신 공통 경로가 필요하다. 부수 관찰: 새 오프닝에서 왕 그룹이 커져 `conversion_capture` 0건, `mine_placement` 5건으로 급감했다. 판정은 저장된 코퍼스에 `--tier-override`를 걸어 **설정당 5초**로 냈다(저장 없으면 45분 재표집). 운영 코드 변경 없음, 문서 기록만.
